...
Con todo esto, evaluamos un total de `r print(total)` modelos de los cuales,  evaluamos  `r   num_regresion ` modelos de regresion multiple,    `r  num_regresion_Ponderada` modelos de regresion ponderada,    `r   num_modelos_GLM ` modelos GLM.    
...
Es un modelo GLM, gamma con funcion liga identidad, el cual obtuvo un AIC de: `r  AIC(modelo) `, donde:
...
Haciendo la verificacion de supuestos mediante pruebas de hipoteises, concluios que: 

+ Nuestro modelo cumple linealidad en las 3 variables,  gracias al test Ramsey de la libreria Car, con un p-value de `r  as.numeric((linealidad[, "Pr(>|Test stat|)"])[1])` verificamos la liealad de bmi, con un p-value `r  as.numeric((linealidad[, "Pr(>|Test stat|)"])[2])` verificamos la de age  y  con un un p-value `r  as.numeric((linealidad[, "Pr(>|Test stat|)"])[2])` la de sex


+ Tambien nuestro modelo cumple homoceasticidad, ya que con un p-value de: `r homoceasticidad$p.value`, concluimo que no eixiste evidencia suficiente para rechar homoceasticidad

+ Ademas, gracias al tamaño de nuestra muestra y a la prueba de hipotesis  Anderson-Darling, con un p-value `r normalidad$p.value`, podemos concluir que nuestros datos se distribuyen de forma normal (Por el tamaño de la prueba y la prueba de hipotesis)

+ Por ultimo gracias a la prueba Breusch-Godfrey con un p-value de `r corResiduos$p.value`, podemos concluir que no hay evidencia que niege la no autocorrelacion de nuestros datos.


Por lo que nuestro modelo cumple todos los supuestos.
